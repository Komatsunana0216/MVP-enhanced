...
def query_prepare5(expid, cleaned_task_data, padding_id, num_queries, trans_mat, feature_df, geometry_df, detour_rate=0.1):
    ...
    feature_df.loc[feature_df['road_speed'] < 1, 'road_speed'] = feature_df['road_speed'].mean()
    feature_df['road_travel_time'] = (feature_df['length'] / feature_df['road_speed']).astype(int)
    history_travel_time_dict = {}
    road_length_dict = {}
    for _, row in feature_df.iterrows():
    ...
    for i in tqdm(range(num_queries), mininterval=10, maxinterval=30):
        row = cleaned_task_data.iloc[random_index[i]]
        road_len=len(row['road_traj'])
        sample_len = int( road_len * detour_rate) + 1
        path = row['road_traj']
        try_count = 0
        sample_pos_list = list(range(1, road_len - sample_len, 1))
        while path == row['road_traj'] and try_count < 10 and len(sample_pos_list) != 0:
            try_count += 1
            start_pos = np.random.choice(sample_pos_list, 1)[0]  # OD不参与处理
            sample_pos_list.remove(start_pos)
            detour_path = row['road_traj'][start_pos:start_pos + sample_len]
            # detour_travel_time = row['road_timestamp'][start_pos+sample_len-1] - row['road_timestamp'][start_pos]
            # origin_path_length = row['total_length']
            path, tm_list = detour(0, row['road_traj'], row['ptime'], detour_path, start_pos)
        ...
        query_route_length.append(len(path))
        q_arr[i, :len(path)] = np.array(path, dtype=np.int32)
        # print("q_arr是什么",q_arr[i, :len(path)])
    ...
    week_arr=torch.LongTensor(week_arr)
    minute_arr = torch.LongTensor(minute_arr)
    # return torch.LongTensor(q_arr), y, np.array(query_route_length), tm_arr
    return torch.LongTensor(q_arr), y, np.array(query_route_length), week_arr,minute_arr
...
# 带时间的 连续子序列detour
def evaluation3(seq_embedding, road_embedding, seq_model, task_data, num_nodes, detour_base, feature_df, geometry_df,road_feature,road_graph, detour_rate, fold=5):
    ...
    for expid in range(fold):
        queries, y, query_route_length, week_arr,minute_arr = query_prepare5(expid, cleaned_split_df, num_nodes, num_queries, detour_base, feature_df,geometry_df, detour_rate)
        # print("queries的形状",queries.shape)
        # 动态获取vocab边界（padding_token - 3 = 实际路段数量 - 1）
        max_road_id = Config.road_special_tokens['padding_token'] - 3
        ...
        for i in range(len(queries)):
            ...
            for item in road_list:
                # 如果road_id超过实际路段数量，设为0
                if int(item) > max_road_id:
                    roadtype=0
                else:
                    roadtype=feature_df.iloc[int(item)]['highway_id']
                tmp.append(roadtype)
            road_type_list.append(tmp)
        ...
        if road_embedding is None:
            route_assign_mat = queries.long().cuda()
            road_traj_inputs=torch.full((route_assign_mat.shape[0], route_assign_mat.shape[1] + 1), Config.road_special_tokens['padding_token'], dtype=torch.long)
            road_week_inputs = torch.zeros((route_assign_mat.shape[0], route_assign_mat.shape[1] + 1), dtype=torch.long)
            road_minute_inputs = torch.zeros((route_assign_mat.shape[0], route_assign_mat.shape[1] + 1), dtype=torch.long)
            road_type_inputs = torch.zeros((route_assign_mat.shape[0], route_assign_mat.shape[1] + 1), dtype=torch.long)
            for i in range(len(route_assign_mat)):

                road_traj_inputs[i, 1:] = route_assign_mat[i]
                road_traj_inputs[i, 0] = Config.road_special_tokens['cls_token']
                road_type_inputs[i, 1:] = torch.LongTensor(road_type_list[i]) + 1
                road_week_inputs[i, 1:] = torch.LongTensor(week_arr[i])
                road_week_inputs[i, 0] = week_arr[i][0]
                road_minute_inputs[i, 1:] = torch.LongTensor(minute_arr[i])
                road_minute_inputs[i, 0] = minute_arr[i][0]
            ...
            with torch.no_grad():
                q_rep_list = []
                for i in range(math.ceil(route_assign_mat.shape[0] / batch_size)):  # 最后不足batch_size的case不要了
                    # math.ceil(route_data.shape[0] / batch_size)
                    start_idx = i * batch_size
                    end_idx = (i + 1) * batch_size
                    if end_idx > route_assign_mat.shape[0]:
                        end_idx = None
                    batch_road_data = {
                    'road_traj': road_traj_inputs[start_idx:end_idx].cuda(),
                    'road_type': road_type_inputs[start_idx:end_idx].cuda(),
                    'road_weeks': road_week_inputs[start_idx:end_idx].cuda(),
                    'road_minutes': road_minute_inputs[start_idx:end_idx].cuda(),
                    }
                    batch_road_data['g_input_feature'] = road_feature
                    batch_road_data['g_edge_index'] = road_graph
                    batch_q = seq_model.routeonly_test(batch_road_data)
                    del batch_road_data
                    q_rep_list.append(batch_q)
                    del batch_q
                    # torch.cuda.empty_cache() # 清空显存

                q = torch.cat(q_rep_list, dim=0)
