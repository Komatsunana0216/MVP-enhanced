...
class Exp:
    def __init__(self, log_path):
        ...
        Config.road_type = self.preprocess.edge.highway_id.nunique()
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            with torch.no_grad():
                ...
                # 记录最佳验证 loss（仅用于日志，不用于 early stopping）
                if average_epoch_eval_loss < self.best_eval_loss:
                    self.best_eval_loss = average_epoch_eval_loss
                    best_epoch = epoch
                
                # 保存最后一个 epoch 的模型配置
                self.last_save_config = tmp_config

        # 禁用 early stopping，直接使用最后一个 epoch 的模型（与 JGRM 保持一致）
        logging.info(f'Training completed! Using last epoch model (epoch {self.epochs})')
        logging.info(f'Best eval loss was at epoch {best_epoch}, but using last epoch for fair comparison with JGRM')
        cache_name = osp.join(self.checkpoint_dir, 'best_pretrain_model.pth')
        torch.save(self.last_save_config, cache_name)
