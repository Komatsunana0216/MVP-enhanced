...
class TrajDataLoader:
    ...
    def _collate_func(self, data_df):
        ...
        road_traj_inputs=torch.full((bz, max_road_len + 1), Config.road_special_tokens['padding_token'], dtype=torch.long)
        # grid_traj_inputs = torch.full((bz, max_grid_len + 1), Config.grid_special_tokens['padding_token'],dtype=torch.long)
        mask_road_traj_inputs = torch.full((bz, max_road_len + 1), Config.road_special_tokens['padding_token'],
                                      dtype=torch.long)
        ...
        for i in range(bz):
            ...
            road_tokens=torch.LongTensor(path)
            road_traj_inputs[i, 1:road_len + 1] = road_tokens
            road_traj_inputs[i, 0] = Config.road_special_tokens['cls_token']

            mask_road_traj = continuous_mask_sequence(road_tokens,
                                                      Config.mask_ratio,
                                                      Config.mask_length)
            ...
            road_temporal = road_temporal_list[i][:-1]
            road_date = [datetime.fromtimestamp(t) for t in road_temporal]
            road_weeks = [d.weekday() + 1 for d in road_date]
            # print("road_weeks",road_weeks)
            # print("road_week_inputs",road_week_inputs[i])
            # print("path",path)
            # print("road_len",road_len)
            # print("road_temporal",road_temporal)
