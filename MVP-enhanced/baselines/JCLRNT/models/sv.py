...
def jsd(z1, z2, pos_mask):
    ...
    # 计算 JSD
    jsd_value = (E_neg * neg_mask).sum() / neg_mask.sum() - (E_pos * pos_mask).sum() / pos_mask.sum()

    # 确保结果非负
    jsd_value = torch.max(jsd_value, torch.tensor(0.0, device=z1.device))
    return jsd_value
...
def ntx(z1, z2, pos_mask, tau=0.5, normalize=True):  # 默认开启归一化
    if normalize:
        z1 = F.normalize(z1, dim=-1)
        z2 = F.normalize(z2, dim=-1)
    sim_mat = torch.mm(z1, z2.t())
...
class SingleViewModel(nn.Module):
    ...
    def encode_sequence(self, sequences, drop_rate=0.):
        ...
        # 添加 eps 防止除以 0 导致 NaN
        seq_pooled = (seq_enc * pool_mask).sum(0) / (pool_mask.sum(0) + 1e-8)
        return seq_pooled
...
def train(data_path, save_path, data_files, num_nodes, edge_index, config):
    ...
    if not config['retrain'] and checkpoints:
        checkpoint_path = os.path.join(save_path, sorted(checkpoints)[-6])
