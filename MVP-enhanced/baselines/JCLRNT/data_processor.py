...
def compute_traj_speed(df, edge_features_path, output_path):
    """
    根据轨迹数据统计每条路段的平均通行速度，更新 edge_features.csv
    使用路段级别的时间间隔计算，比原来的方法更准确
    
    Args:
        df: 包含轨迹数据的DataFrame，需要包含 'cpath_list' 和 'road_interval' 字段
        edge_features_path: edge_features.csv 文件路径
        output_path: 输出文件路径
    """
    print("\n" + "=" * 60)
    print("计算并更新 traj_speed (改进版)")
    print("=" * 60)
    
    edge_features = pd.read_csv(edge_features_path)
    print(f"读取 edge_features: {len(edge_features)} 条路段")
    
    road_speeds = defaultdict(list)
    
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="统计路段速度"):
        cpath = row['cpath_list']
        
        # 检查是否有 road_interval 字段
        if 'road_interval' in row:
            road_intervals = row['road_interval']
        elif 'pass_time' in row:
            # 兼容旧格式：如果只有 pass_time，使用它
            road_intervals = row['pass_time'] if isinstance(row['pass_time'], list) else eval(row['pass_time'])
        else:
            # 如果没有路段级别的时间信息，跳过这条轨迹
            continue
        
        for i, road_id in enumerate(cpath):
            if road_id < len(edge_features) and i < len(road_intervals):
                length = edge_features.loc[road_id, 'length']
                interval = road_intervals[i]
                
                if interval > 0 and length > 0:
                    speed = length / interval
                    # 过滤异常值：速度在 0.1-50 m/s 之间（约 0.36-180 km/h）
                    if 0.1 < speed < 50:
                        road_speeds[road_id].append(speed)
    
    traj_speed_values = []
    for road_id in range(len(edge_features)):
        speeds = road_speeds.get(road_id, [])
        if speeds:
            avg_speed = np.mean(speeds)
        else:
            # 如果没有轨迹经过，使用 road_speed 作为默认值
            avg_speed = edge_features.loc[road_id, 'road_speed'] if 'road_speed' in edge_features.columns else 0.0
        traj_speed_values.append(avg_speed)
    
    edge_features['traj_speed'] = traj_speed_values
    edge_features.to_csv(output_path, index=False)
    print(f"已更新 traj_speed 并保存到: {output_path}")
    
    covered_roads = sum(1 for speeds in road_speeds.values() if speeds)
    print(f"有轨迹经过的路段数: {covered_roads} / {len(edge_features)} ({100*covered_roads/len(edge_features):.1f}%)")
