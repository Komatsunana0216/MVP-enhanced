...
def query_prepare5(expid, cleaned_task_data, padding_id, num_queries, trans_mat, feature_df, geometry_df,vocab, detour_rate=0.1):
    np.random.seed(expid)
    ...
    random_index = np.random.permutation(num_samples)
    q_arr = np.full([num_queries, max_len], padding_id, dtype=np.int32)
    before_q_arr = np.full([num_queries, max_len], padding_id, dtype=np.int32)
    week_arr = np.full([num_queries, max_len], 0, dtype=np.int32)
    minute_arr = np.full([num_queries, max_len], 0, dtype=np.int32)
    timestamp_arr=np.full([num_queries, max_len], 0, dtype=np.int32)
    prefix = cleaned_task_data[:num_queries,0, :]
    batch_temporal_mat = np.zeros((num_queries, max_len+1, max_len+1))
    query_route_length = []
    for i in tqdm(range(num_queries)):
        row= cleaned_task_data[random_index[i]]
        # row = cleaned_task_data[i]


        origin_path = row[1:,0].tolist()
        origin_tm=row[1:,1].tolist()

        len_path = len([element for element in origin_path if element != -999])
        sample_len = int(len_path * detour_rate) + 1

        try_count = 0
        sample_pos_list = list(range(1, len_path - sample_len, 1))
        origin_path = origin_path[:len_path]
        path=origin_path
        while path == origin_path and try_count < 10 and len(sample_pos_list) != 0:
            try_count += 1
            start_pos = np.random.choice(sample_pos_list, 1)[0]  # OD不参与处理
            sample_pos_list.remove(start_pos)
            detour_path = origin_path[start_pos:start_pos + sample_len]


            path, tm_list = detour(0, origin_path, origin_tm, detour_path, start_pos)
        ...
        before_path=path
        # print("字典什么样",vocab.loc2index)
        # print("反字典什么样", vocab.index2loc)

        path=[vocab.loc2index.get(item,-1) for item in path]
        q_arr[i, :len(path)] = np.array(path, dtype=np.int32)
        before_q_arr[i, :len(path)] = np.array(before_path, dtype=np.int32)
        week_arr[i, :len(path)] = np.array(week_list, dtype=np.int32)
        minute_arr[i, :len(path)] = np.array(minute_list, dtype=np.int32)
        timestamp_arr[i, :len(path)]=np.array(tm_list[:-1], dtype=np.int32)
        query_route_length.append(len(path))

        tm_list = [tm_list[0]] + tm_list[:-1]
        seq_len = len(tm_list)
        mat = np.zeros((seq_len, seq_len))
        for i in range(seq_len):
            for j in range(seq_len):
                off = abs(tm_list[i] - tm_list[j])
                mat[i][j] = off

        batch_temporal_mat[i, :len(path)+1, :len(path)+1] = mat



    y = random_index[:num_queries]

    tm_arr = torch.cat([torch.LongTensor(minute_arr).unsqueeze(dim=2),torch.LongTensor(week_arr).unsqueeze(dim=2)], dim=-1)

    query_route_length= torch.tensor(query_route_length, dtype=torch.int16)
    batch_size = query_route_length.numel()

    padding_mask=(torch.arange(0, max_len+1)
            .type_as(query_route_length)
            .repeat(batch_size, 1)
            .lt(query_route_length.unsqueeze(1)))
    return torch.LongTensor(q_arr), y, torch.LongTensor(timestamp_arr), tm_arr,torch.LongTensor(batch_temporal_mat),padding_mask,prefix,torch.LongTensor(before_q_arr)
...
# 带时间的 连续子序列detour
def evaluation3(seq_embedding, seq_model, task_data, num_nodes, detour_base, feature_df, geometry_df, vocab,graph_dict,device,origin_X,detour_rate, fold=5):
    ...
    for expid in range(fold):
        queries, y, timestamp, tm_mat,temporal_mat,padding_masks,prefix,before_q = query_prepare5(expid, cleaned_split_df, num_nodes, num_queries,
                                                                detour_base, feature_df, geometry_df,vocab, detour_rate)
        # print("qshape",queries.shape)
        # print("timestampshape",timestamp.shape)
        # print("tm_matshape",tm_mat.shape)
        X=torch.cat([queries.unsqueeze(dim=2),timestamp.unsqueeze(dim=2),tm_mat],dim=-1)
        X = X.to(device)
        # print("prefix",prefix.shape)
        X=torch.cat([prefix.unsqueeze(dim=1),X],dim=1)
        # print(X.shape)



        padding_masks = padding_masks.to(device)  # 0s: ignore
        # print("temporal_mat的形状",temporal_mat.shape)

        temporal_mat = temporal_mat.to(device)


        # print("detour之前", origin_X[:2, :, :])
        # print(origin_X.shape)
        # print("detour之前转换后", task_data[:2, :, :])
        # print("detour之后转换前", before_q[:2, :])
        # print("detour之后",X[:2,:,:])
        # print(X.shape)

        q=[]
        with torch.no_grad():
            for i in range(math.ceil(X.shape[0] / batch_size)):  # 最后不足batch_size的case不要了
                # print("进行到batch",i)
                ...
                batch_temporal_mat = temporal_mat[start_idx:end_idx]
                batch_padding_masks = padding_masks[start_idx:end_idx]
                batch_q,_ = seq_model(x=batch_X, padding_masks=batch_padding_masks, batch_temporal_mat=batch_temporal_mat,
                              graph_dict=graph_dict)

                del batch_X, batch_temporal_mat, batch_padding_masks
