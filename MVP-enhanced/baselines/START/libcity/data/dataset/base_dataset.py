...
class BaseDataset(AbstractDataset):
    def __init__(self, config):
        ...
        # self.roadnetwork = self.config.get('roadnetwork', 'chengdu_road_edge')
        self.roadnetwork = self.config.get('roadnetwork', 'xian_road_edge')
    ...
    def _load_vocab(self):
        ...
        self.logger.info('vocab_path={},  vocab_size={}'.format(
            self.vocab_path,  self.vocab_size))
    ...
    def _load_geo_feature(self, road_info):
        ...
        if os.path.exists(node_fea_path):
            ...
            useful = ['highway_id', 'lanes', 'length']
            if self.append_degree2gcn:
            ...
            for k, v in norm_dict.items():
                ...
                node_features = node_features.drop(columns=k)
                node_features.insert(v, k, dnew)
            # onehot_list = ['lanes', 'maxspeed', 'highway']
            onehot_list = ['lanes',  'highway_id']
            if self.append_degree2gcn:
...
class TrajectoryProcessingDataset(Dataset):
    ...
    def data_processing(self, origin_data, desc=None, cache_path=None, tmat_path=None):
        ...
        sub_data = origin_data[['path', 'tlist']]
        ...
        for i in tqdm(range(sub_data.shape[0]), desc=desc):
            ...
            # tim_list=tim_list[:-1]  # 注释掉：porto_new 数据中 path 和 tlist 长度已经一致
            # print(loc_list)
            # usr_id = traj['usr_id']
