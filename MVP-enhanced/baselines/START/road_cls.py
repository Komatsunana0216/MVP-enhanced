...
def evaluation(model, feature_df,road_list, fold=100):
# def evaluation(model, feature_df, fold=100):
    x = model

    valid_labels = ['primary', 'secondary', 'tertiary', 'residential']
    #todo 改了分类数
    # valid_labels=['living_street', 'motorway', 'motorway_link', 'planned', 'trunk',
    #                     'secondary', 'trunk_link', 'tertiary_link', 'primary', 'residential',
    #                     'primary_link', 'unclassified', 'tertiary', 'secondary_link']
    id_dict = {idx: i for i, idx in enumerate(valid_labels)}
    # y_df = feature_df.loc[feature_df['highway'].isin(valid_labels)]
    y_df = feature_df.loc[(feature_df['highway'].isin(valid_labels)) & (feature_df['fid'].isin(road_list))]
    x = x[y_df['fid'].tolist()]
    y = torch.tensor(y_df['highway'].map(id_dict).tolist())
    print("要做多少数据的分类",len(y))
    ...
    for i in range(fold):
        ...
        # 计算类别权重以处理类别不平衡（使用更强的权重）
        from collections import Counter
        import numpy as np
        class_counts = Counter(y_train.numpy().tolist())
        total_samples = len(y_train)
        # 使用 inverse frequency 加权，并应用平方根来平衡权重强度
        class_weights = torch.tensor([
            np.sqrt(total_samples / class_counts.get(j, 1))
            for j in range(len(valid_labels))
        ], dtype=torch.float32).cuda()
        # 归一化权重
        class_weights = class_weights / class_weights.sum() * len(valid_labels)
        if i == 0:  # 只打印一次
            print(f"Class weights: {class_weights.cpu().numpy()}")
            print(f"Class counts in first fold: {dict(class_counts)}")
        ...
        for e in range(1, 101):
            model.train()
            ce_loss = nn.CrossEntropyLoss(weight=class_weights)(model(x_train), y_train.cuda())
    ...
    per_class_f1 = f1_score(y_trues, y_preds, average=None, labels=list(range(len(valid_labels))))
    per_class_precision = precision_score(y_trues, y_preds, average=None, labels=list(range(len(valid_labels))))
    per_class_recall = recall_score(y_trues, y_preds, average=None, labels=list(range(len(valid_labels))))
    
    print(f'\nroad classification     | micro F1: {micro_f1:.4f}, macro F1: {macro_f1:.4f}')
    print('\nPer-class metrics:')
    print(f"{'Class':<12} {'Precision':<12} {'Recall':<12} {'F1':<12}")
    print("-" * 48)
    for idx, label in enumerate(valid_labels):
        print(f"{label:<12} {per_class_precision[idx]:<12.4f} {per_class_recall[idx]:<12.4f} {per_class_f1[idx]:<12.4f}")
    
    # 输出混淆矩阵（简化版）
    cm = confusion_matrix(y_trues, y_preds, labels=list(range(len(valid_labels))))
