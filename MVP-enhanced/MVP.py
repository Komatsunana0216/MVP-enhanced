...
class MVPModel(BaseModel):
    def __init__(self, prompt_ST,vocab_size, route_max_len, road_feat_num, road_embed_size, gps_feat_num, gps_embed_size, route_embed_size, hidden_size, edge_index, drop_edge_rate, drop_route_rate, drop_road_rate,  bert_hiden_size, pad_token_id,
        ...
        self.bert_config = BertConfig(
            ...
            attn_implementation="eager"  # 显式指定attention实现方式，避免版本兼容性问题
        )
        self.seg_embedding_learning = BertForMaskedLM(self.bert_config)
        # 修复新版 transformers 兼容性问题：手动设置 BertModel 的 attn_implementation 属性
        # 需要同时设置带下划线和不带下划线的版本，以兼容不同版本的 transformers
        if hasattr(self.seg_embedding_learning, 'bert'):
            self.seg_embedding_learning.bert._attn_implementation = "eager"
            self.seg_embedding_learning.bert.attn_implementation = "eager"
    ...
    def encode_route(self, route_data, route_assign_mat, masked_route_assign_mat):
        ...
        # 创建一个形状相同的张量用于存放标签
        label_tensor = torch.full_like(route_assign_mat, -100).cuda()
        ...
        if self.prompt_ST == 1:
            ...
            loss_1 = self.seg_embedding(
                [bert_emb, encoder_mask.to(torch.long), label_tensor.to(torch.long)])
        else:
            loss_1 = self.seg_embedding(
    ...
    def encode_gps(self, gps_data, masked_gps_assign_mat, masked_route_assign_mat, gps_length,route_assign_mat):
        ...
        transformer_padding_mask = (new_route_assign == self.vocab_size)
        if stacked_gps_emb.shape[1]!=transformer_padding_mask.shape[1]:
            print("stacked_gps_emb",stacked_gps_emb.shape)
            print("transformer_padding_mask",transformer_padding_mask.shape)
            print("route_length",route_length)
            print(max(route_length.values()))
            print(new_route_assign.shape)
            print(route_assign_mat.shape)
            max_key = max(route_length, key=lambda k: route_length[k])
            print(route_assign_mat[max_key])
    ...
    def encode_joint(self, route_road_rep, route_traj_rep, gps_road_rep, gps_traj_rep, route_assign_mat, gps_assign_mat):
        ...
        for i, length in enumerate(route_length):
            ...
            if pos_emb.shape!=gps_emb.shape:
                print(pos_emb.shape)
                print(gps_emb.shape)
                print(route_assign_mat[i])
                print(gps_assign_mat[i])
    ...
    def forward(self, route_data, masked_route_assign_mat, gps_data, masked_gps_assign_mat, route_assign_mat, gps_length):
        gps_road_rep, gps_traj_rep = self.encode_gps(gps_data, masked_gps_assign_mat, masked_route_assign_mat, gps_length,route_assign_mat)
        route_road_rep, route_traj_rep, loss1 = self.encode_route(route_data, route_assign_mat, masked_route_assign_mat)
        gps_road_joint_rep, gps_traj_joint_rep, route_road_joint_rep, route_traj_joint_rep = self.encode_joint(route_road_rep, route_traj_rep,
                                                                                        ...
                                                                                        route_assign_mat,masked_gps_assign_mat)
        return gps_road_rep, gps_traj_rep, route_road_rep, route_traj_rep, \
