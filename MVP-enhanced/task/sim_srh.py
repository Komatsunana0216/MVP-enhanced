...
import torch
import math
from evluation_utils import get_seq_emb_from_node,get_seq_emb_from_traj_withRouteOnly,get_seq_emb_from_traj_withALLModel
...
def query_prepare5(expid, cleaned_task_data, padding_id, num_queries, trans_mat, feature_df, geometry_df, detour_rate=0.1):
    ...
    def dfs_path(start, end, detour_path, origin_path, max_iterations=10000):
        stack = [(start, [start])]
        paths = []
        iteration_count = 0  # 添加迭代计数器，防止无限循环
        while stack:
            iteration_count += 1
            if iteration_count > max_iterations:  # 超过最大迭代次数，返回已找到的路径或None
                if len(paths) > 0:
                    return paths[-1]
                return None
    ...
    def detour(replace_rate, path, tm_list, detour_path, start_pos):
        ...
        #         if count==end_pos:
        #             gps_end_pos=i
        #     gps_path[gps_start_pos:gps_end_pos]=padding_id
        #     for j in range(len(gps_data)):
        #         gps_data[j][gps_start_pos:gps_end_pos]=-1
        ...
        # # print(gps_end_pos)
        # # print(gps_path)
        # gps_path=gps_path[:gps_start_pos]+detour_anchor+gps_path[gps_end_pos:]
        # gps_length = _split_duplicate_subseq(gps_path)
        # for j in range(len(gps_data)):
        #     gps_data[j]=gps_data[j][:gps_start_pos]+[-1] * len(detour_anchor)+gps_data[j][gps_end_pos:]
    ...
    # q_gps_arr = np.full([num_queries, gps_max_len], padding_id, dtype=np.int32)
    # q_gpslen_arr = np.full([num_queries, max_len], 0, dtype=np.int32)
    # q_gpsdata_arr = np.full([num_queries, 8, gps_max_len], 0, dtype=np.float32)

    query_route_length = []
    for i in tqdm(range(num_queries), mininterval=10, maxinterval=30):
        row = cleaned_task_data.iloc[random_index[i]]
        sample_len = int(row['route_length'] * detour_rate) + 1
        path = row['cpath_list']
        # gps_path=row['opath_list']
        # gps_data=row[['tm_list', \
        #      'lng_list', 'lat_list', \
        #      'speed', 'acceleration', \
        #      'angle_delta', 'interval', \
        #      'dist']]
        #
        # def _split_duplicate_subseq(opath_list):
        #     length_list = []
        ...
        while path == row['cpath_list'] and try_count < 10 and len(sample_pos_list) != 0:
            ...
            # path, tm_list,gps_path,gps_length,gps_data = detour(0, row['cpath_list'], row['road_timestamp'], detour_path, start_pos,row['opath_list'],row[['tm_list',\
            #                                               'lng_list', 'lat_list',\
            #                                               'speed', 'acceleration',\
            #                                               'angle_delta', 'interval',\
            #                                               'dist']])
        ...
        # q_gps_arr[i, :len(gps_path)] = np.array(gps_path, dtype=np.int32)
        # # print("gps_path",gps_path)
        # # print("gps_length",gps_length)
        #
        # q_gpslen_arr[i, :len(path)] = np.array(gps_length, dtype=np.int32)
        # # print("gps_data",gps_data)
        # for j in range(8):
        #     gps_data[j]=np.nan_to_num(gps_data[j], nan=0)
        #     q_gpsdata_arr[i,j, :len(gps_path)] = np.array(gps_data[j])
    ...
    # return torch.LongTensor(q_arr), y, np.array(query_route_length), tm_arr,torch.LongTensor(q_gps_arr),torch.LongTensor(q_gpslen_arr),torch.FloatTensor(q_gpsdata_arr)
    return torch.LongTensor(q_arr), y, np.array(query_route_length), tm_arr
...
def evaluation(seq_embedding, road_embedding, seq_model, task_data, num_nodes, detour_base, detour_rate, fold=5):
    ...
    x = seq_embedding.detach().cpu().numpy()
...
# 带时间的 连续子序列detour
def evaluation3(seq_embedding, road_embedding, seq_model, task_data, num_nodes, detour_base, feature_df, geometry_df, detour_rate, fold=5, get_seq_emb_fn=None):
    """
    Args:
        get_seq_emb_fn: 可选的序列嵌入获取函数。如果为None，默认使用 get_seq_emb_from_traj_withRouteOnly。
                        对于 GPS-only 模型，应该传入 get_seq_emb_from_traj_gps_only 或类似函数。
    """
    print('device: cpu')
    batch_size = 1024
    num_queries = 5000
    ...
    for expid in range(fold):
        # queries, y, query_route_length, tm_mat,queries_gps,queries_gps_length,queries_gps_data = query_prepare5(expid, cleaned_split_df, num_nodes, num_queries, detour_base, feature_df,geometry_df, detour_rate)
        ...
        # queries_gps_data=queries_gps_data.permute(0, 2, 1)
        # queries_gps_data[:, :, 0] = torch.ones_like(queries_gps_data[:, :, 0])
        # print(queries_gps_data)
        # for i in range(1, queries_gps_data.shape[2]):
        #     # for i in range(1, gps_data.shape[2]-3):
        #     fea = queries_gps_data[:, :, i]
        #     occupied_mask = fea == -1
        #     mask = (fea != -1) & (fea != 0)
        #     # 根据掩码选择元素
        #     non_zero_minus_one_fea = torch.masked_select(fea, mask)
        #     queries_gps_data[:, :, i] = (queries_gps_data[:, :, i] - torch.mean(non_zero_minus_one_fea)) / torch.std(non_zero_minus_one_fea)
        #     queries_gps_data[:, :, i][occupied_mask]=-1
        # print(queries_gps_data)
        # print(queries_gps_data.shape)
        ...
        if road_embedding is None:
            ...
            route_data = tm_mat.long().cuda()
            queries = (route_data, route_assign_mat, None, None, route_assign_mat, None, None)
            # queries = (route_data, route_assign_mat, queries_gps_data, queries_gps, route_assign_mat, queries_gps_length, None)
            # 使用传入的函数或默认的 get_seq_emb_from_traj_withRouteOnly
            seq_emb_fn = get_seq_emb_fn if get_seq_emb_fn is not None else get_seq_emb_from_traj_withRouteOnly
            q = seq_emb_fn(seq_model, queries, batch_size=1024)
