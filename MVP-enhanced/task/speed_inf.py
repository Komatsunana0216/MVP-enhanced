...
# def evaluation(embedding, feature_df, fold=5):
def evaluation(embedding, feature_df, road_list, fold=5, city='chengdu'):
    # x = embedding
    y_df = feature_df.loc[(feature_df['fid'].isin(road_list))]
    x = embedding[y_df['fid'].tolist()]
    
    # 波尔图数据集使用 traj_speed，其他数据集使用 road_speed
    if city == 'porto':
        speed_column = 'traj_speed'
    else:
        speed_column = 'road_speed'
    
    y = torch.tensor(y_df[speed_column].tolist())
    print(f'[INFO] 速度推断使用标签列: {speed_column}')
    # y = torch.tensor(feature_df['road_speed'].tolist())

    # x = [embedding[token] for token in road_list]
    #
    # # 筛选出对应的标签
    # y = torch.tensor(
    #     feature_df.loc[feature_df.index[road_list], 'road_speed'].tolist())  # 假设feature_df的索引与tokens直接对应
