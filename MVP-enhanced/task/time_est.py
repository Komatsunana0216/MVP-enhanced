...
def data_prepare(task_data, padding_id):
    ...
    split_df = task_data.copy()  # 'path', 'total_time' 或从 road_timestamp 计算
    split_df['path_len'] = split_df['cpath_list'].map(len)
    split_df = split_df.loc[(split_df['path_len'] > min_len) & (split_df['path_len'] < max_len)]
    
    # 兼容不同数据集的列名：优先使用 total_time，否则从 road_timestamp 或 tm_list 计算
    if 'total_time' not in split_df.columns:
        if 'road_timestamp' in split_df.columns:
            split_df['total_time'] = split_df['road_timestamp'].apply(lambda x: x[-1] - x[0] if len(x) > 1 else 0)
        elif 'tm_list' in split_df.columns:
            split_df['total_time'] = split_df['tm_list'].apply(lambda x: x[-1] - x[0] if len(x) > 1 else 0)
        else:
            raise KeyError("数据中缺少 'total_time', 'road_timestamp' 或 'tm_list' 列，无法计算旅行时间")
