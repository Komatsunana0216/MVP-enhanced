...
def extract_weekday_and_minute_from_list(timestamp_list):
    """
    从时间戳列表中提取 weekday 和 minute
    自动检测：如果时间戳是 2013-2014 年（Porto 数据），使用 UTC 时区
    否则使用本地时区（成都/西安数据）
    """
    weekday_list = []
    minute_list = []
    
    # 检测是否是 Porto 数据（2013-2014年的时间戳范围）
    # Porto: 1372636800 (2013-07-01) ~ 1406851200 (2014-08-01)
    # 成都/西安: 1541030400 (2018-11-01) ~ 以后
    first_ts = timestamp_list[0] if len(timestamp_list) > 0 else 0
    use_utc = 1370000000 < first_ts < 1420000000  # 2013-2014 年范围
    
    for timestamp in timestamp_list:
        if use_utc:
            dt = datetime.utcfromtimestamp(timestamp)  # Porto 数据用 UTC
        else:
            dt = datetime.fromtimestamp(timestamp)  # 成都/西安用本地时区
...
def prepare_gps_data(df,mat_padding_value,data_padding_value,max_len):
    ...
    # print("opath_list",opath_list[0])
    gps_assign_mat = rnn_utils.pad_sequence(opath_list, padding_value=mat_padding_value,
                                                               batch_first=True)
    # print("gps_assign_mat", gps_assign_mat[0])
    # print("gps_assign_mat的形状",gps_assign_mat.shape)
    ...
    # print("gps_data",gps_data[0])
    # print("gps_data的形状",gps_data.shape)
...
def prepare_route_data(df,mat_padding_value,data_padding_value,max_len):
    ...
    route_assign_mat = rnn_utils.pad_sequence(cpath_list, padding_value=mat_padding_value,
                                                               batch_first=True)
    # print("在这里时候是什么",route_assign_mat)
    # print("在这里的时候的形状",route_assign_mat.shape)
    # padding route data
    ...
    # print("route_data",route_data[0])
    # print("route_data的形状",route_data.shape)
...
class StaticDataset(Dataset):
    def __init__(self, data, mat_padding_value, data_padding_value,gps_max_len,route_max_len):
        ...
        #                                                   'speed', 'acceleration',\
        #                                                   'angle_delta', 'interval',\
        #                                                   'dist','date_list','week_list','slot_list']], mat_padding_value, data_padding_value, gps_max_len)
...
def get_train_loader(data_path, batch_size, num_worker, route_min_len, route_max_len, gps_min_len, gps_max_len, num_samples, seed):
    ...
    dataset = pd.read_pickle(open(data_path, 'rb'))
    print(dataset.columns)
    ...
    mat_padding_value = max(uniuqe_path_list) + 1
    print("mat_padding_value", mat_padding_value)
    data_padding_value = 0.0
...
def random_mask(gps_assign_mat, route_assign_mat, gps_length, mask_token, mask_length=1, mask_prob=0.2):
    ...
    # print("截断前route_mask_pos",route_mask_pos[0])
    ...
    # print("masked_route_assign_mat",masked_route_assign_mat[0])
    # for i in range(len(route_assign_mat)):
        # print("mat_padding_value",mat_padding_value)
        # print("mat_padding_value形状",mat_padding_value.shape)
        # gps_non_zero = (gps_length[i] != 0).sum().item()
        # route_non_padding = (route_assign_mat[i] != mat_padding_value[0]).sum().item()
        # if gps_non_zero != route_non_padding:
        #     print("发现不等长opath和cpath")
        #     print("gps_assign_mat",gps_assign_mat[i])
        #     print("route_assign_mat",route_assign_mat[i])
        #     a
        # mask gps
    ...
    for idx, row in enumerate(gps_assign_mat):
        ...
        # print("unpad_mask_pos_list",unpad_mask_pos_list)
        # print(len(unpad_mask_pos_list))
