...
def finetune(config):
    ...
    # load model
    # PyTorch 2.6+ 需要添加 weights_only=False 来加载包含自定义类的模型
    model = torch.load(model_path, map_location="cuda:{}".format(dev_id), weights_only=False)['model']
    for param in model.parameters():
        param.requires_grad = False

    model.prompt_ST=1
    model.route_prompt = prompt_class.prompt(vocab_size,edge_features,length_mean,length_std,route_embed_size)
    model.route_prompt.requires_grad_(True)
    model.route_prompt.cuda()
    model.route_fuse=nn.Linear(route_embed_size, route_embed_size)
    model.route_fuse.requires_grad_(True)
    ...
    model.bert_prompt.requires_grad_(True)
...
if __name__ == '__main__':
    city = 'porto'  # 修改为 porto 使用最后一个 epoch 模型进行 finetune
