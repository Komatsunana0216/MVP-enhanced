...
# def get_traj_match_loss(gps_traj_rep, route_traj_rep,gps_traj_joint_rep,route_traj_joint_rep, model, batch_size=64, tau=0.07):
def get_traj_match_loss(gps_traj_rep, route_traj_rep, model,batch_size=64, tau=0.07):
    ...
    #todo 这里改了
    # sim_targets = torch.zeros(sim_g2r.size()).cuda()
    # sim_targets.fill_diagonal_(1)
    #
    #
    # # sim_i2t_targets = alpha * F.softmax(sim_i2t_m, dim=1) + (1 - alpha) * sim_targets
    # # sim_t2i_targets = alpha * F.softmax(sim_t2i_m, dim=1) + (1 - alpha) * sim_targets
    # loss_i2t = -torch.sum(F.log_softmax(sim_g2r, dim=1) * sim_targets, dim=1).mean()
    # loss_t2i = -torch.sum(F.log_softmax(sim_r2g, dim=1) * sim_targets, dim=1).mean()
   ...
   #  pos_pair_pre = torch.cat([route_traj_rep, gps_traj_rep], dim=1)
   #  neg_pair1_pre = torch.cat([route_traj_rep_neg, gps_traj_rep], dim=1)
   #  neg_pair2_pre = torch.cat([route_traj_rep, gps_traj_rep_neg], dim=1)
   #
   #  all_pair_pre = torch.cat([pos_pair_pre, neg_pair1_pre, neg_pair2_pre], dim=0)
   #
   #  pred_pre = model.matching_predictor_pre(all_pair_pre) # 2 x 64*3
   #
   #  label = torch.cat([torch.ones(batch_size, dtype=torch.long), torch.zeros(2 * batch_size, dtype=torch.long)],dim=0).cuda() # 1 x 64*3
   #  loss_itm_pre = F.cross_entropy(pred_pre, label)
   #
   # #todo 从这里开始是改的，下面的注释是原来的
   #  # select a negative route for each gps
   #  route_joint_traj_rep_neg = []
   #  for i in range(batch_size):
   #      neg_idx = torch.multinomial(weight_g2r[i], 1).item()
   #      route_joint_traj_rep_neg.append(route_traj_joint_rep[neg_idx])
   #  route_joint_traj_rep_neg = torch.stack(route_joint_traj_rep_neg, dim=0)
   #
   #  # select a negative gps for each route
   #  gps_joint_traj_rep_neg = []
   #  for i in range(batch_size):
   #      neg_idx = torch.multinomial(weight_r2g[i], 1).item()
   #      gps_joint_traj_rep_neg.append(gps_traj_joint_rep[neg_idx])
   #  gps_joint_traj_rep_neg = torch.stack(gps_joint_traj_rep_neg, dim=0)
   #
   #
   #
   #  # 每一个GR pair都有两个负样本 GR’ 和 G‘R，flat之后分为3组，GR,GR',G'R 64*3 x 256
   #
   #  pos_pair = torch.cat([route_traj_joint_rep, gps_traj_joint_rep], dim=1)
   #  neg_pair1 = torch.cat([route_joint_traj_rep_neg, gps_traj_joint_rep], dim=1)
   #  neg_pair2 = torch.cat([gps_traj_joint_rep, gps_joint_traj_rep_neg], dim=1)
   #
   ...
   #
   #  label = torch.cat([torch.ones(batch_size, dtype=torch.long), torch.zeros(2 * batch_size, dtype=torch.long)],dim=0).cuda() # 1 x 64*3
   #  loss_itm = F.cross_entropy(pred, label)
   #  # return loss_itc,loss_itm
   #  return loss_itm_pre,loss_itm
